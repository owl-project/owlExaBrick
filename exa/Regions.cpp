...
namespace exa {
  ...
  void ExaBrickRegions::addLeaf(std::vector<std::pair<box3f,int>> &buildPrims,
                                const box3f &domain)
  {
    ...
    brickRegions.push_back(newLeaf);    
  }
    
  void ExaBrickRegions::buildRec(std::vector<std::pair<box3f,int>> &buildPrims,
                                 const box3f &domain)
  {
  ...
  void ExaBrickRegions::computeValueRange(BrickRegion &region,
                                          ExaBricks::SP exa,
                                          const exa::Brick *deviceBricks,
                                          const float *scalarBuffers,
                                          const unsigned *scalarBufferOffsets,
                                          size_t numScalarBuffers)
  {
    region.valueRange = range1f();
    for (size_t f=0;f<numScalarBuffers;++f) {
      for (int i=0;i<region.leafListSize;i++) {
        int brickID = leafList[region.leafListBegin+i];
        const Brick &brick = deviceBricks[brickID];
  ...
  void ExaBrickRegions::buildFrom(ExaBricks::SP exa,
                                  ...
                                  size_t numScalarBuffers)
  {
...

    {
      ...
      for (int i=0;i<numBricks;i++)
        stat_numCells += deviceBricks[i].numCells();
    }
    ...
    serial_for_blocked(0,numBricks,4*1023,[&](size_t begin, size_t end){
        ...
        box3f blockBounds;
        for (size_t i=begin;i<end;i++) {
            box3f domain = exa->bricks[i]->getDomain();
    ...
    parallel_for(brickRegions.size(),[&](size_t regionID){
        // for (auto &region : brickRegions) {
        auto &region = brickRegions[regionID];
        int finestLevel = 1<<30;
        for (int i=0;i<region.leafListSize;i++) {
          int brickID = leafList[region.leafListBegin+i];
          const ExaBricks::Brick &brick = *exa->bricks[brickID];
          finestLevel = std::min(finestLevel,brick.level);
        }
        region.finestLevelCellWidth = 1<<finestLevel;
        computeValueRange(region,exa,deviceBricks,scalarBuffers,scalarBufferOffsets,numScalarBuffers);
    ...
    std::cout << "total num leaves " << prettyDouble(brickRegions.size()) << std::endl;
    std::cout << "avg leaf size " << (leafList.size() / float(brickRegions.size())) << std::endl;
